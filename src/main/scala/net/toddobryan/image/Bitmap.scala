...
object Bitmap {
  ...
  def apply(path: String, name: Option[String] = None): Image = {
    ensureInitialized(new Bitmap(Left(new File(path)), name))
  }
  ...
  def fromUrl(url: String, name: Option[String] = None): Bitmap = {
    ensureInitialized(new Bitmap(Right(new URL(url)), name))
  }
  ...
  private[image] def fromWorkspace(path: String, name: Option[String] = None): Bitmap = {
    ensureInitialized(new Bitmap(Right(getClass.getResource(path)), name))
  }
}